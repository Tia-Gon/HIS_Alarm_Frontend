...
import 'package:simple_webapp/app/domain/choice_item/choice_item.dart';
import 'package:simple_webapp/app/domain/choice_item/item.dart';
...
import 'package:simple_webapp/app/domain/pis_data/chart_page.dart';
import 'package:simple_webapp/app/domain/signup/address_detail_page.dart';
import 'package:simple_webapp/app/domain/signup/signup_page.dart';

import '../domain/main/main_page.dart';
...
class AppRouter {
  ...
  GoRouter _getAppRouter() => GoRouter(
        ...
        routes: [
          ...
          GoRoute(
            path: AppRouteConstants.choice,
            name: AppRouteConstants.choice,
            builder: (context, state) => const ChoiceItem(),
          ),
          GoRoute(
            path: AppRouteConstants.item,
            name: AppRouteConstants.item,
            builder: (context, state) => const Item(),
          ),
          GoRoute(
              path: AppRouteConstants.addressDetail,
              name: AppRouteConstants.addressDetail,
              builder: (context, state) => const AddressDetailPage()),
          GoRoute(
              path: AppRouteConstants.chart,
              name: AppRouteConstants.chart,
  ...
  Future<void> setLoginUserInfo(Ref ref) async {
    ...
    if (appService.secureStorageService.accessToken.isNotEmpty &&
      ...
      appService.loginUser = await ref
          .read(UserApi.provider)
          .getUser(email: appService.secureStorageService.loginedUserId);
    }
  }
